                    ...
                    <div class="space-y-6">
                        @foreach ($team->users->sortBy('name') as $user)
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center">
                                    {{-- 1. TEAM OWNER HANDLING --}}
                                    @if ($team->user_id === $user->id)
                                        <div class="ms-2 text-sm text-gray-400">
                                            <div class="flex items-center gap-2 py-1.5">
                                                <x-role-badge role="owner"/>
                                            </div>
                                        </div>

                                        {{-- 2. EDITABLE MEMBER ROLE (If you have permission) --}}
                                    @elseif (Gate::check('updateTeamMember', $team) && Laravel\Jetstream\Jetstream::hasRoles())
                                        <button class="ms-2 text-sm text-gray-400 underline"
                                            ...
                                            <div class="flex items-center gap-2 px-3 py-1.5">
                                                {{-- Use $user->membership->role to get the specific user's role --}}
                                                <x-role-badge :role="$user->membership->role"/>
                                            </div>
                                        </button>

                                        {{-- 3. READ-ONLY MEMBER ROLE --}}
